<?php
/**
 * Admin - Creer une etape de la frise
...
$data = ['year' => '', 'title' => '', 'description' => '', 'sort_order' => ''];
$errors = [];

// Prochain sort_order par defaut
$last = dbFetchOne('SELECT MAX(sort_order) as max_order FROM timeline_entries');
$data['sort_order'] = ($last['max_order'] ?? 0) + 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        dbExecute(
            ...
            [$data['year'], $data['title'], $data['description'], $data['sort_order']]
        );
        header('Location: ' . admin_url('timeline') . '?success=Etape ajoutee avec succes');
        exit;
    }
}

$pageTitle = 'Ajouter une etape';
...
<form method="post" class="admin-form">
    ...
    <div class="form-actions">
        <button type="submit" class="btn btn-primary">Ajouter l'etape</button>
